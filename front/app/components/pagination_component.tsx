...
const PaginationComponent = () => {
  ...
  useEffect(() => {
    if (prevId) {
      router.prefetch(
        routePath.LESSON_DETAIL.replace(
          "[courseId]",
          courseId as string,
        ).replace("[lessonId]", prevId.toString()),
      );
    }
    if (postId) {
      router.prefetch(
        routePath.LESSON_DETAIL.replace(
          "[courseId]",
          courseId as string,
        ).replace("[lessonId]", postId.toString()),
      );
    }
  }, [courseId, postId, prevId, router]);

  return (
    <footer className="fixed bottom-0 left-0 right-0 flex justify-between bg-gray-200 py-4">
